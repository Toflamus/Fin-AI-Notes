...
\begin{frame}{核心 Takeaways}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Kyle 模型是理论之锚}——价格冲击 $\lambda$ 是信息风险的均衡定价

    \item \textbf{VPIN 是风控的眼睛}——体积时钟让我们提前一小时看见闪电崩盘

    \item \textbf{参与者画像揭示战场}——冰山 / 扫单 / Boehmer 散户构成完整对手盘地图

    \item \textbf{OFI 是终极武器}——线性预测 $\Delta P$ 的强度堪比物理定律

    \item \textbf{深度学习是未来}——多级 LOB + CNN/LSTM 突破传统因子的天花板
  \end{enumerate}
...
\begin{frame}{实习个人收获}
  ...
  \begin{itemize}
    \item \textbf{从宏观视角到微观视角的跨越}——不再只看 K 线,而是看每一笔订单背后的人和算法

    \item \textbf{从因子使用者到因子构建者}——理解 OFI 不是一个黑盒,而是有清晰物理意义的事件聚合

    \item \textbf{数据基础设施的重要性}——MBO / L2 / Tick 数据的获取、清洗、对齐是一切微观研究的前提

    \item \textbf{统计严谨性的训练}——t 统计量、IC/IR、多重检验校正,避免被表面数据骗

    \item \textbf{工程与理论的平衡}——再优雅的模型,也要在实时风控的延迟约束下落地
  \end{itemize}

  \vfill

  \begin{exampleblock}{下一步}
    深入研究多级 OFI 的稳定性、跨资产 OFI 的联合建模、以及 DeepLOB 在中国市场的适配性。
  \end{exampleblock}
\end{frame}
